...
import { y0Locations } from '../core/y0-locations.js';
...
export function setupLocationSearch() {
	...
	// y0 Residency quick-pick locations
	const y0Grid = document.getElementById('y0-location-grid');
	if (y0Grid) {
		y0Locations.forEach(loc => {
			...
			btn.addEventListener('click', () => {
				updateState({
					city: loc.name.toUpperCase(),
					country: loc.country.toUpperCase(),
					lat: loc.lat, lon: loc.lon, zoom: loc.zoom,
					markers: [{ lat: loc.lat, lon: loc.lon }],
					routeStartLat: loc.lat, routeStartLon: loc.lon,
					routeEndLat: loc.lat - 0.005, routeEndLon: loc.lon + 0.005,
					routeViaPoints: [], routeGeometry: []
				});
				updateMapPosition(loc.lat, loc.lon, loc.zoom);
				updateMarkerStyles(state);
				if (state.showRoute) {
					updateRouteGeometry().then(() => updateRouteStyles(state));
				}
				...
				// Highlight active
				y0Grid.querySelectorAll('.y0-loc-btn').forEach(b => {
